      ...
      <div className="max-w-5xl mx-auto flex flex-col relative z-10 w-full">
        ...
        <div className="flex flex-col items-end mb-16 w-full">
          ...
          <div className="w-full text-right">
            <h2 className="text-white text-xl md:text-2xl font-medium leading-[1.5] tracking-wider opacity-90 uppercase italic">
              {t.description}
            </h2>
          </div>
        </div>

        {/* Stats Pill - აქ გასწორდა flex-row და gap */}
        <motion.div 
        ...
        >
          {t.stats.map((stat: any, index: number) => (
            <div key={index} className="text-center relative flex-1 w-full group">
              <motion.h3 
              ...
              <p className="text-white/90 text-[10px] md:text-sm font-bold uppercase tracking-[0.3em] whitespace-nowrap">
                {stat.label}
              </p>
              
              {/* გამყოფი ხაზი */}
              {index !== t.stats.length - 1 && (
                <div className="hidden md:block absolute -right-0 top-1/2 -translate-y-1/2 h-14 w-[1px] bg-white/20"></div>
              )}
            </div>
          ))}
        </motion.div>
